...
test("buildIndexPage: 头部统计 + fork 森林嵌套列表 + 树间空行", () => {
  const root = makeItem({ mdBase: "2026-09-14-001-根", name: "根会话", messageCount: 100, modified: new Date("2026-09-14T01:00:00Z") });
  const child = makeItem({
    mdBase: "2026-09-15-001-子",
    name: "子会话",
    messageCount: 50,
    modified: new Date("2026-09-15T01:00:00Z"),
    parentSessionPath: `/tmp/x/sessions/--p--/${root.mdBase}.jsonl`,
  });
  // 另一棵树：整体更新（应排前面）
  const other = makeItem({ mdBase: "2026-09-16-001-另", name: "另一棵", messageCount: 10, modified: new Date("2026-09-16T01:00:00Z") });
  // root.path 需与 child.parentSessionPath 的 basename 配对（makeItem 默认 path 已含 mdBase）
  const text = buildIndexPage([root, child, other], stubResolve);
  const lines = text.split("\n");

  assert.equal(lines[0], "# 会话索引 — proj");
  assert.match(lines[2], /共 3 条对话 · 2 棵会话树 · 项目 `\/tmp\/proj`/);
  // 最近活跃的树（另一棵）排最前
  ...
  // 嵌套缩进 + 链接与元数据格式
  assert.match(lines[iOther], /^- \[另一棵\]\(<\.\/2026-09-16-001-另\.l3\.md>\) — 10 msgs · 1KB · \d{4}-\d{2}-\d{2}$/);
  assert.match(lines[iRoot], /^- \[根会话\]\(<\.\//);
  assert.match(lines[iChild], /^ {2}- \[子会话\]\(<\.\//);
  // 两棵树之间有空行
...
test("buildIndexPage: 无 cwd → 标题无项目后缀", () => {
...
function mdFixture(fm: { id: string; name?: string; cwd?: string; source: string; parent?: string; ended?: string }): string {
  ...
  lines.push(`started: ${q("2026-09-14T01:00:00.000Z")}`, `ended: ${q(fm.ended ?? "2026-09-14T02:00:00.000Z")}`);
  if (fm.name) lines.push(`name: ${q(fm.name)}`);
  lines.push("messages:", "  user: 2", "  assistant: 2", `source: ${q(fm.source)}`, "---", "", "## 🙋 User · 2026-09-14 09:00", "", "你好", "");
  return lines.join("\n");
}
...
test("generateDirIndex: 从 md 产物生成 index.md；重复生成 changed=false；空目录 → null", async () => {
  ...
  fs.writeFileSync(
    ...
    mdFixture({ id: "uuid-child", name: "子会话", cwd: "/tmp/proj", source: "child.jsonl", parent: "/tmp/x/sessions/--tmp-proj--/root.jsonl", ended: "2026-09-15T02:00:00.000Z" }),
  );
  ...
  assert.ok(content.includes("共 2 条对话 · 1 棵会话树"), "统计行");
